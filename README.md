...
- ✅ `README.md` is maintained and upgraded (not removed).
- ✅ Full WAProto declaration coverage is available (`WAProto/**/*.d.ts`).
- ✅ CI branch target uses `main` (not `master`).
- ✅ `sendGroupStatus(...)` follows status/story audience flow (`status@broadcast`) for better compatibility.
- ✅ Newsletter/channel media path uses a single configurable flow (`forceNewsletterMedia`) to avoid duplicated logic.
