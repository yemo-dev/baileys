...
const getMccForPhoneNumber = (phoneNumber, iso3166Alpha2) => {
    ...
    const normalizedInputDigits = normalizeDialingCode(phoneNumber);
    if (!normalizedInputDigits) {
        return country ? (0, exports.getMccForCountryIso2)(country) : '000';
    }
    let parsedDigits;
    try {
        const parsed = (0, libphonenumber_js_1.parsePhoneNumberFromString)(String(phoneNumber), country);
        if (parsed?.countryCallingCode) {
            parsedDigits = normalizeDialingCode(`${parsed.countryCallingCode}${parsed.nationalNumber || ''}`);
        }
    }
    catch (_e) {
    }
    const mcc = resolveMccByDialingDigits(parsedDigits || normalizedInputDigits);
    if (mcc !== undefined) {
        return String(mcc).padStart(3, '0');
    }
    return country ? (0, exports.getMccForCountryIso2)(country) : '000';
