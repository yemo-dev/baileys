...
const getMccForPhoneNumber = (phoneNumber, iso3166Alpha2) => {
    ...
    let phoneDigits = normalizeDialingCode(phoneNumber);
    if (!phoneDigits) {
    ...
    try {
        ...
        if (parsed?.countryCallingCode) {
            phoneDigits = normalizeDialingCode(`${parsed.countryCallingCode}${parsed.nationalNumber || ''}`);
        }
    }
    catch (_e) {
    }
    const mcc = resolveMccByDialingDigits(phoneDigits);
    if (mcc !== undefined) {
        return String(mcc).padStart(3, '0');
    }
    return country ? (0, exports.getMccForCountryIso2)(country) : '000';
