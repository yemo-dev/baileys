...
function makeLibSignalRepository(auth, logger, pnToLIDFunc) {
    ...
    return {
        ...
        async migrateSession(fromJid, toJid) {
            ...
            const toLid = (0, WABinary_1.transferDevice)(fromJid, toJid);
            const fromAddr = jidToSignalProtocolAddress(fromJid).toString();
            const toAddr = jidToSignalProtocolAddress(toLid).toString();
            const { [fromAddr]: sess } = await auth.keys.get('session', [fromAddr]);
            if (!sess) {
                return { migrated: 0, skipped: 1, total: 1 };
            }
            await auth.keys.set({ session: { [toAddr]: sess, [fromAddr]: null } });
...
const jidToSignalProtocolAddress = (jid) => {
    ...
    const device = (decoded === null || decoded === void 0 ? void 0 : decoded.device) || 0;
    return new libsignal.ProtocolAddress(user, device);
};
