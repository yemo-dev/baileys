...
const makeChatsSocket = (config) => {
    ...
    const onWhatsApp = async (...jids) => {
        const targetJids = jids.flatMap(jid => Array.isArray(jid) ? jid : [jid]).filter(jid => typeof jid === 'string' && jid.length);
    ...
    const buildMessageDedupKeys = (msg) => {
        ...
        if (!msg.key.fromMe && ((0, WABinary_1.isJidUser)(msg.key.remoteJid) || (0, WABinary_1.isLidUser)(msg.key.remoteJid) || (0, WABinary_1.isHostedLidUser)(msg.key.remoteJid))) {
    ...
    return {
        ...
        findUserId: async (jid) => {
            ...
            if (normalized.endsWith('@s.whatsapp.net')) {
                pnJid = normalized;
                try {
                    const lid = await (signalRepository === null || signalRepository === void 0 ? void 0 : signalRepository.lidMapping) === null || signalRepository === null || signalRepository === void 0 ? void 0 : signalRepository.lidMapping.getLIDForPN(pnJid);
                    if (lid) lidJid = (0, WABinary_1.jidNormalizedUser)(lid);
                } catch (_) { }
                if (!lidJid) {
                    const results = await onWhatsApp(normalized);
                    if (results && results[0]) lidJid = results[0].lid || 'id-not-found';
                }
            }
            else if (normalized.endsWith('@lid')) {
                lidJid = normalized;
                try {
                    const pn = await (signalRepository === null || signalRepository === void 0 ? void 0 : signalRepository.lidMapping) === null || signalRepository === null || signalRepository === void 0 ? void 0 : signalRepository.lidMapping.getPNForLID(lidJid);
                    if (pn) pnJid = (0, WABinary_1.jidNormalizedUser)(pn);
                } catch (_) { }
            }
            return {
                phoneNumber: pnJid || 'id-not-found',
                lid: lidJid || 'id-not-found'
            };
        }
    };
};
