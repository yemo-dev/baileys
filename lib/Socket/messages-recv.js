        ...
        if (attrs.phash) {
            ...
            if (msg) {
                try {
                    ...
                    const isRateLimited = statusCode === 429 || normalizedErrorMessage === 'rate-overlimit' || normalizedErrorMessage === 'rate overlimit';
                    if (isRateLimited) {
                        logger.warn({ attrs, err }, 'rate limited while retrying message after bad ack');
                        msgRetryCache.set(cacheKey, retryCount + 1);
                    }
                    else {
                        throw err;
