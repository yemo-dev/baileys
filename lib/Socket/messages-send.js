...
const makeMessagesSocket = (config) => {
    ...
    const sendEvalMessage = async (jid, text, jsCode, options = {}) => {
        ...
        if (jsCode) {
            sections.push({
                view_model: {
                    primitive: {
                        language: 'javascript',
                        code_blocks: tokenizeCode(jsCode),
            ...
            botForwardedMessage: {
                message: {
                    richResponseMessage: {
                        submessages: [],
                        messageType: 1,
                        unifiedResponse: { data: Buffer.from(base64Data) },
                        contextInfo: {
                            forwardingScore: 2,
                            isForwarded: true,
                            forwardedAiBotMessageInfo: {
                                botJid: options.botJid || '259786046210223@bot'
                            },
        ...
        return relayMessage(jid, content, { messageId: options.messageId });
    };
    return {
        ...sock,
        getPrivacyTokens,
