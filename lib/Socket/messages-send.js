...
const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, useUserDevicesCache, useCachedGroupMetadata, statusJidList, additionalNodes }) => {
        ...
        const hasGroupStatusWrapper = !!(normalizedMessage?.groupStatusMessageV2 || normalizedMessage?.groupStatusMessage);
        let effectiveStatusJidList = statusJidList;
        const decodedInitialJid = (0, WABinary_1.jidDecode)(jid);
        if (decodedInitialJid?.server === 'g.us' && hasGroupStatusWrapper) {
            if (!effectiveStatusJidList?.length) {
                const { allUsers } = await resolveStatusAudience([jid], false);
                effectiveStatusJidList = allUsers;
            }
            if (effectiveStatusJidList?.length) {
                logger.debug({ jid, audience: effectiveStatusJidList.length }, 'routing groupStatus wrapper message through status@broadcast');
                jid = WABinary_1.STORIES_JID;
            }
            else {
                logger.warn({ jid }, 'failed to resolve audience for groupStatus wrapper message');
            }
        }
