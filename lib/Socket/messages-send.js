...
const makeMessagesSocket = (config) => {
    ...
    const generateStatusMessage = async (content, generationOptions = {}) => {
    ...
    const sendStatusToResolvedAudience = async (content, jids = [], opts = {}) => {
        ...
        const mentionNodes = mentionedJids.length
            ? [{
                    tag: 'meta',
                    attrs: {},
                    content: [
        ...
        for (const jid of mentionedJids) {
            ...
            try {
                ...
                logger.warn({ jid: normalizedId, trace: error?.stack || String(error) }, 'failed to relay status mention envelope, continuing');
            }
    ...
    const sendStatusMentions = async (content, jids = []) => sendStatusToResolvedAudience(content, jids);
    const sendGroupStatus = async (groupIdsOrContent = [], contentOrGroups = {}, opts = {}) => {
        const MIN_GROUP_ID_DIGITS = 8;
        const normalizeGroupJid = (value) => {
            ...
            if (!trimmed.includes('@') && new RegExp(`^\\d{${MIN_GROUP_ID_DIGITS},}$`).test(trimmed)) {
                return `${trimmed}@g.us`;
            }
            return trimmed;
        };
        const toGroupIdArray = (value) => {
            if (Array.isArray(value)) {
                return value.map(normalizeGroupJid).filter(Boolean);
            }
            if (typeof value === 'string' && value) {
                const normalized = normalizeGroupJid(value);
        ...
        const nonGroupJids = groupIds.filter(jid => !(0, WABinary_1.isJidGroup)((0, WABinary_1.jidNormalizedUser)(jid)));
        ...
        await sendStatusToResolvedAudience(content, groupJids, sendOpts);
        return groupJids;
    };
    const sendAlbumMessage = async (jid, medias, options = {}) => {
        ...
        for (const i in medias) {
            ...
            const albumItemAttrs = {};
            if (mediaHandle) {
                albumItemAttrs['media_id'] = mediaHandle;
            }
            await relayMessage(jid, msg.message, { messageId: msg.key.id, additionalAttributes: albumItemAttrs });
            await (0, Utils_1.delay)(time);
        }
        return album;
    };
    const waitForMsgMediaUpdate = (0, Utils_1.bindWaitForEvent)(ev, 'messages.media-update');
    const shouldUseNewsletterMediaRoute = (jid, options = {}) => (0, WABinary_1.isJidNewsletter)(jid) && !(options.forceNewsletterMedia !== undefined ? options.forceNewsletterMedia : forceNewsletterMedia);
    return {
        ...sock,
        getPrivacyTokens,
        assertSessions,
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' && 'album' in content && content.album) {
                ...
                for (const i in album) {
                    ...
                    if ('image' in media) {
                        mediaMsg = await (0, Utils_1.generateWAMessage)(jid, {
                            ...
                            userJid,
                            upload: async (readStream, opts) => {
                                const up = await waUploadToServer(readStream, { ...opts, newsletter: shouldUseNewsletterMediaRoute(jid, options) });
                                mediaHandle = up.handle;
                                return up;
                            },
                            ...options,
                    ...
                    const sendMsgAttrs = {};
                    if (mediaHandle) {
                        sendMsgAttrs['media_id'] = mediaHandle;
                    }
                    await relayMessage(jid, mediaMsg.message, {
                        messageId: mediaMsg.key.id,
                        additionalAttributes: sendMsgAttrs
                    });
                ...
                const fullMsg = await (0, Utils_1.generateWAMessage)(jid, content, {
                    ...
                    ...options,
                    forceNewsletterMedia: options.forceNewsletterMedia !== undefined ? options.forceNewsletterMedia : forceNewsletterMedia,
                });
