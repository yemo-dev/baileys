...
const getRandomHexColor = () => "#" + Math.floor(Math.random() * MAX_RGB_VALUE).toString(16).padStart(6, "0");
const makeMessagesSocket = (config) => {
        ...
        for (const rawJid of jids) {
            ...
            if (server === 'g.us') {
                try {
                    ...
                    logger.warn({ jid: normalizedJid, trace: error?.stack || String(error) }, 'failed to resolve group metadata for status audience, skipping group');
                }
    ...
    const sendStatusMentions = async (content, jids = []) => {
        ...
        let msg = await generateStatusMessage({
            ...
            font: (() => {
                if (typeof content.font === 'number' && content.font >= 0 && content.font < STATUS_FONT_COUNT) {
                    return content.font;
                }
                if (typeof content.font === 'number') {
                    logger.warn({ font: content.font }, `invalid status font index, expected 0-${STATUS_FONT_COUNT - 1}`);
                }
    ...
    const sendGroupStatus = async (content, groups = []) => {
        const groupJids = groups
            .map(jid => (0, WABinary_1.jidNormalizedUser)(jid))
            .filter(jid => (0, WABinary_1.isJidGroup)(jid));
        const filteredOutJids = groups.filter(jid => !(0, WABinary_1.isJidGroup)((0, WABinary_1.jidNormalizedUser)(jid)));
        if (filteredOutJids.length) {
            logger.warn({ filteredOutJids }, 'ignoring non-group JIDs in sendGroupStatus');
        }
        const { allUsers } = await resolveStatusAudience(groupJids, false);
        const msg = await generateStatusMessage(content);
        await relayMessage(WABinary_1.STORIES_JID, msg.message, {
            messageId: msg.key.id,
            statusJidList: allUsers,
        });
