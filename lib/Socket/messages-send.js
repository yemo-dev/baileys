...
const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, useUserDevicesCache, useCachedGroupMetadata, statusJidList, additionalNodes }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (!isNewsletter && buttonType) {
                ...
                const hasBizBotNode = (0, WABinary_1.getBinaryFilteredBizBot)(additionalNodes ? additionalNodes : []);
                if (filteredButtons) {
    ...
    const getButtonArgs = (message) => {
        ...
        if (nativeFlow && (firstButtonName === 'review_and_pay' || firstButtonName === 'payment_info')) {
            ...
            const isProductList = message.listMessage.listType === WAProto_1.proto.Message.ListMessage.ListType.PRODUCT_LIST;
            return {
                tag: 'biz',
                attrs: {
                    actual_actors: '2',
                ...
                content: [{
                        ...
                        attrs: {
                            v: '2',
                            type: isProductList ? 'product_list' : 'select'
                        }
                    },
                    {
                        tag: 'quality_control',
