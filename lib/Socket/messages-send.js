        ...
        for (const rawJid of jids) {
            ...
            if (server === 'g.us') {
                try {
                    ...
                    logger.warn({ jid: normalizedJid, trace: error?.stack || String(error) }, 'failed to resolve group metadata for status audience, skipping group and its members; verify membership/permissions');
                }
