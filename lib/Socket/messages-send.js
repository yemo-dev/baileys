...
const makeMessagesSocket = (config) => {
    ...
    const sendGroupStatus = async (groupIdsOrContent = [], contentOrGroups = {}, opts = {}) => {
        ...
        for (const gid of groupJids) {
            ...
            let mediaHandle;
            const inside = await (0, Utils_1.generateWAMessageContent)(content, {
                logger,
                upload: async (readStream, uploadOpts) => {
                    const up = await waUploadToServer(readStream, { ...uploadOpts });
            ...
            const msg = (0, Utils_1.generateWAMessageFromContent)(gid, {
                messageContextInfo: { messageSecret },
                groupStatusMessageV2: {
                    message: {
                        ...inside,
                        messageContextInfo: {
                            ...((inside === null || inside === void 0 ? void 0 : inside.messageContextInfo) || {}),
                            messageSecret
                        },
                    },
                },
            ...
            await relayMessage(gid, msg.message, {
                messageId: msg.key.id,
                additionalAttributes,
                ...(sendOpts.relay || {}),
            });
