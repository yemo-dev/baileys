...
const makeMessagesSocket = (config) => {
    ...
    const resolveStatusAudience = async (jids = []) => {
        const userJid = (0, WABinary_1.jidNormalizedUser)(authState.creds.me.id);
        ...
        for (const rawJid of jids) {
            ...
            if (server === 'g.us') {
                try {
                    ...
                    logger.warn({ jid: normalizedJid, trace: error?.stack || String(error) }, 'failed to resolve group metadata for status audience');
                }
                normalizedMentions.push(normalizedJid);
            }
        ...
        return {
            ...
            mentionedJids: [...new Set(normalizedMentions)]
        };
    };
    const sendStatusMentions = async (content, jids = []) => {
        ...
        let msg = await generateStatusMessage({
            ...content,
            backgroundColor: content.backgroundColor || ("#" + Math.floor(Math.random() * 16777215).toString(16).padStart(6, "0")),
            font: typeof content.font === 'number' ? content.font : Math.floor(Math.random() * STATUS_FONT_COUNT),
        });
    ...
    const sendGroupStatus = async (content, groups = []) => {
        ...
        const { allUsers } = await resolveStatusAudience(groupJids);
