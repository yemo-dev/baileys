...
const isExpectedSessionMiss = (err) => !!(err && typeof err.message === 'string' && err.name === 'SessionError' && err.message.includes('No matching sessions found for message'));
...
async function getDecryptionJidCandidates(stanza, sender, repository) {
    const candidates = [];
    const primary = await getDecryptionJid(sender, repository);
    pushUnique(candidates, primary);
    pushUnique(candidates, sender);
    const { senderAlt } = extractAddressingContext(stanza);
    pushUnique(candidates, senderAlt);
    if (repository.lidMapping) {
        if ((0, WABinary_1.isLidUser)(sender) || (0, WABinary_1.isHostedLidUser)(sender)) {
            const mappedPn = await repository.lidMapping.getPNForLID(sender);
            pushUnique(candidates, mappedPn);
        }
        else if ((0, WABinary_1.isPnUser)(sender) || (0, WABinary_1.isHostedPnUser)(sender)) {
            const mappedLid = await repository.lidMapping.getLIDForPN(sender);
            pushUnique(candidates, mappedLid);
        }
    }
    return candidates;
}
                ...
                for (const { tag, attrs, content } of stanza.content) {
                    ...
                    try {
                        ...
                        switch (e2eType) {
                            ...
                            case 'pkmsg':
                            case 'msg': {
                                const decryptionJidCandidates = await getDecryptionJidCandidates(stanza, author, repository);
                                const decryptionJid = decryptionJidCandidates[0] || author;
                                if (tag !== 'plaintext') {
                                    await storeMappingFromEnvelope(stanza, author, repository, decryptionJid, logger);
                                }
                                let lastSessionMissErr;
                                for (const candidateJid of decryptionJidCandidates) {
                                    try {
                                        ...
                                        if (isExpectedSessionMiss(err)) {
                                            lastSessionMissErr = err;
                                            continue;
                                        }
                                        throw err;
                                    }
                                ...
                                if (!msgBuffer) {
                                    throw (lastSessionMissErr || new Error('No matching sessions found for message'));
                                }
                                break;
                            }
                            case 'plaintext':
                        ...
                        if (isExpectedSessionMiss(err)) {
                            logger.debug({ key: fullMessage.key, err }, 'session missing for message, waiting retry flow');
                        }
                        else {
                            logger.error({ key: fullMessage.key, err }, 'failed to decrypt message');
                        }
