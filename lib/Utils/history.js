    ...
    switch (item.syncType) {
        ...
        case WAProto_1.proto.HistorySync.HistorySyncType.ON_DEMAND:
            for (const chat of item.conversations) {
                ...
                if (chat.tcToken && chat.tcToken.length) {
                    const tcEntry = { jid: chat.id, token: new Uint8Array(chat.tcToken) };
                    if (chat.tcTokenTimestamp) {
                        tcEntry.timestamp = (0, generics_1.toNumber)(chat.tcTokenTimestamp);
                    }
                    if (chat.tcTokenSenderTimestamp) {
                        tcEntry.senderTimestamp = (0, generics_1.toNumber)(chat.tcTokenSenderTimestamp);
                    }
