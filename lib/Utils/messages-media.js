...
exports.getStatusCodeForMediaRetry = exports.decryptMediaRetryData = exports.decodeMediaRetryNode = exports.encryptMediaRetryRequest = exports.getWAUploadToServer = exports.downloadEncryptedContent = exports.downloadContentFromMessage = exports.getUrlFromDirectPath = exports.encryptedStream = exports.prepareStream = exports.getHttpStream = exports.getStream = exports.toBuffer = exports.toReadable = exports.mediaMessageSHA256B64 = exports.generateProfilePicture = exports.transcodeAudio = exports.encodeBase64EncodedStringForUpload = exports.extractImageThumb = exports.extractVideoThumb = exports.hkdfInfoKey = void 0;
...
const getImageProcessingLibrary = async () => {
    ...
    if (_jimp) {
        ...
        const JimpClass = (jimp === null || jimp === void 0 ? void 0 : jimp.Jimp) || jimp;
        return { jimp: JimpClass };
    }
...
const extractImageThumb = async (bufferOrFilePath, width = 32) => {
    ...
    if ('sharp' in lib && typeof ((_a = lib.sharp) === null || _a === void 0 ? void 0 : _a.default) === 'function') {
        ...
        const { read, MIME_JPEG, RESIZE_BILINEAR, RESIZE_BEZIER, AUTO } = lib.jimp;
        const jimp = await read(bufferOrFilePath);
        const getW = typeof jimp.getWidth === 'function' ? jimp.getWidth() : jimp.bitmap.width;
        const getH = typeof jimp.getHeight === 'function' ? jimp.getHeight() : jimp.bitmap.height;
        const dimensions = { width: getW, height: getH };
        const resizeAlgo = RESIZE_BILINEAR !== null && RESIZE_BILINEAR !== void 0 ? RESIZE_BILINEAR : RESIZE_BEZIER;
        const autoVal = AUTO !== null && AUTO !== void 0 ? AUTO : -1;
        const mime = MIME_JPEG !== null && MIME_JPEG !== void 0 ? MIME_JPEG : 'image/jpeg';
        const resized = jimp.quality(50).resize(width, autoVal, resizeAlgo);
        const buffer = typeof resized.getBufferAsync === 'function'
            ? await resized.getBufferAsync(mime)
            : await resized.getBuffer(mime);
        return {
            buffer,
            original: dimensions
        };
...
const generateProfilePicture = async (mediaUpload) => {
    ...
    if ('sharp' in lib && typeof ((_c = lib.sharp) === null || _c === void 0 ? void 0 : _c.default) === 'function') {
        ...
        const size = Math.min(meta.width || 550, meta.height || 550);
        const buffer = await img
    ...
    if (!lib.jimp && !jimp_1.default) {
        throw new boom_1.Boom('No image processing library available');
    }
    const JimpClass = (lib.jimp) || jimp_1.default;
    const jimpImg = await JimpClass.read(bufferOrFilePath);
    const imgW = typeof jimpImg.getWidth === 'function' ? jimpImg.getWidth() : jimpImg.bitmap.width;
    const imgH = typeof jimpImg.getHeight === 'function' ? jimpImg.getHeight() : jimpImg.bitmap.height;
    const cropped = imgW > imgH ? jimpImg.resize(550, -1) : jimpImg.resize(-1, 650);
    const mime = (JimpClass.MIME_JPEG) || 'image/jpeg';
    const imgBuf = typeof cropped.getBufferAsync === 'function'
        ? await cropped.quality(100).getBufferAsync(mime)
        : await cropped.quality(100).getBuffer(mime);
    return { img: imgBuf };
};
...
async function getAudioDuration(buffer) {
    ...
    if (Buffer.isBuffer(buffer)) {
        metadata = await musicMetadata.parseBuffer(buffer, options);
    }
    else if (typeof buffer === 'string') {
        metadata = await musicMetadata.parseFile(buffer, options);
    }
    else {
        metadata = await musicMetadata.parseStream(buffer, options);
    }
...
async function transcodeAudio(audio, options = {}) {
    ...
    if (Buffer.isBuffer(audio)) {
        ...
        const rStream = (0, fs_1.createReadStream)(audio);
        audioData = await (0, exports.toBuffer)(rStream);
    }
    else {
        audioData = await (0, exports.toBuffer)(audio);
    }
    ...
    return async (stream, { mediaType, fileEncSha256B64, newsletter, timeoutMs }) => {
        ...
        for (const { hostname, maxContentLengthBytes } of hosts) {
            ...
            try {
                ...
                result = body.data;
                if ((result === null || result === void 0 ? void 0 : result.url) || (result === null || result === void 0 ? void 0 : result.directPath) || (result === null || result === void 0 ? void 0 : result.handle)) {
                    urls = {
                        mediaUrl: result.url,
