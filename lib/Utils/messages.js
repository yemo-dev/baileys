...
exports.readViewOnce = exports.assertMediaContent = exports.downloadMediaMessage = exports.aggregateMessageKeysNotFromMe = exports.updateMessageWithPollUpdate = exports.updateMessageWithReaction = exports.updateMessageWithReceipt = exports.getDevice = exports.extractMessageContent = exports.normalizeMessageContent = exports.getContentType = exports.generateWAMessage = exports.generateWAMessageFromContent = exports.generateWAMessageContent = exports.generateForwardMessageContent = exports.prepareDisappearingMessageSettingContent = exports.prepareWAMessageMedia = exports.generateLinkPreviewIfRequired = exports.extractUrlFromText = void 0;
exports.getAggregateVotesInPollMessage = getAggregateVotesInPollMessage;
exports.updateMessageWithEventResponse = exports.getAggregateResponsesInEventMessage = void 0;
...
const prepareWAMessageMedia = async (message, options) => {
    ...
    const useNewsletterMediaRoute = !!options.newsletter && !options.forceNewsletterMedia;
    const { mediaKey, encWriteStream, bodyPath, fileEncSha256, fileSha256, fileLength, didSaveToTmpPath, } = await (useNewsletterMediaRoute ? messages_media_1.prepareStream : messages_media_1.encryptedStream)(uploadData.media, options.mediaTypeOverride || mediaType, {
        logger,
        saveOriginalFileIfRequired: requiresOriginalForSomeProcessing,
        opts: options.options
    });
    
    const fileEncSha256B64 = (useNewsletterMediaRoute ? fileSha256 : fileEncSha256 !== null && fileEncSha256 !== void 0 ? fileEncSha256 : fileSha256).toString('base64');
...
const normalizeButtonsMessageButton = (button) => {
    var _a, _b, _c, _d;
    const buttonTextObject = button.buttonText && typeof button.buttonText === 'object'
        ? button.buttonText
        : undefined;
    ...
    const displayText = displayTextCandidates.find(value => typeof value === 'string' && value.length > 0) || '';
    const buttonId = button.buttonId || button.id || ((_a = button.buttonParamsJson) === null || _a === void 0 ? void 0 : _a.id) || '';
    const nativeFlowInfoInput = ((_b = button.nativeFlowInfo) !== null && _b !== void 0 ? _b : button.nativeFlow) || button.nativeFlows;
    const hasNativeFlowInfo = !!nativeFlowInfoInput || button.type === 4 || button.type === WAProto_1.proto.Message.ButtonsMessage.Button.Type.NATIVE_FLOW;
    return {
        buttonId,
        buttonText: { displayText },
        type: hasNativeFlowInfo
            ? WAProto_1.proto.Message.ButtonsMessage.Button.Type.NATIVE_FLOW
            : WAProto_1.proto.Message.ButtonsMessage.Button.Type.RESPONSE,
        ...(hasNativeFlowInfo
            ? {
                nativeFlowInfo: {
                    name: (_c = nativeFlowInfoInput === null || nativeFlowInfoInput === void 0 ? void 0 : nativeFlowInfoInput.name) !== null && _c !== void 0 ? _c : '',
                    paramsJson: (_d = nativeFlowInfoInput === null || nativeFlowInfoInput === void 0 ? void 0 : nativeFlowInfoInput.paramsJson) !== null && _d !== void 0 ? _d : '',
                }
            }
            : {})
    };
...
const DEFAULT_EMBED_MUSIC_POLYGON_VERTICES = [
    { x: 0.25, y: 0.25 },
    { x: 0.75, y: 0.25 },
    { x: 0.75, y: 0.75 },
    { x: 0.25, y: 0.75 },
];
const DEFAULT_EMBED_MUSIC_CONTENT = {
    musicContentMediaId: '0',
    songId: '0',
    author: 'yebail',
    title: 'yebail',
    artistAttribution: 'yebail'
};
...
const applyMediaAnnotations = (m, message) => {
    const annotationsInput = Array.isArray(message.annotations)
        ? message.annotations
        : (Array.isArray(message.interactiveAnnotations) ? message.interactiveAnnotations : undefined);
    if (!annotationsInput || annotationsInput.length === 0) {
        return;
    }
    const annotations = annotationsInput.map(annotation => {
        ...
        if (!('embeddedAction' in normalized) && !('tapAction' in normalized) && !('location' in normalized) && !('newsletter' in normalized)) {
            normalized.embeddedAction = true;
        }
        if (!Array.isArray(normalized.polygonVertices) || normalized.polygonVertices.length === 0) {
            const hasEmbeddedMusic = !!(normalized.embeddedContent
                && typeof normalized.embeddedContent === 'object'
                && normalized.embeddedContent.embeddedMusic
                && typeof normalized.embeddedContent.embeddedMusic === 'object');
            normalized.polygonVertices = hasEmbeddedMusic
                ? DEFAULT_EMBED_MUSIC_POLYGON_VERTICES
                : [
                    { x: 0, y: 0 },
                    { x: 1, y: 0 },
                    { x: 1, y: 1 },
        ...
        if (normalized.embeddedContent
            && typeof normalized.embeddedContent === 'object'
            && normalized.embeddedContent.embeddedMusic
            && typeof normalized.embeddedContent.embeddedMusic === 'object') {
            normalized.embeddedContent = {
                ...normalized.embeddedContent,
                embeddedMusic: {
                    ...DEFAULT_EMBED_MUSIC_CONTENT,
                    ...normalized.embeddedContent.embeddedMusic
                }
            };
        }
    ...
    const canHaveAnnotations = messageType === 'imageMessage' || messageType === 'videoMessage';
    if (!canHaveAnnotations) {
        return;
    }
    m[messageType].interactiveAnnotations = annotations;
    m[messageType].annotations = annotations;
};
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
        ...
        if ('musicMetadata' in message && !!message.musicMetadata) {
            extContent.musicMetadata = message.musicMetadata;
        }
        ...
        m.commentMessage = {
            ...
            targetMessageKey: message.comment.key ?? message.comment.targetMessageKey
        };
    }
        ...
        m.eventMessage = {};
        const startTime = message.event.startDate
            ? Math.floor(message.event.startDate.getTime() / 1000)
            : (message.event.startTime != null ? Number(message.event.startTime) : undefined);
        if (message.event.call && options.getCallLink) {
            const token = await options.getCallLink(message.event.call, { startTime });
            m.eventMessage.joinLink = (message.event.call === 'audio'
        ...
        m.eventMessage.endTime = message.event.endDate
            ...
            : (message.event.endTime != null ? Number(message.event.endTime) : undefined);
        m.eventMessage.isCanceled = message.event.isCancelled ?? message.event.isCanceled ?? false;
        m.eventMessage.extraGuestsAllowed = message.event.extraGuestsAllowed;
        m.eventMessage.isScheduleCall = message.event.isScheduleCall ?? false;
        m.eventMessage.location = message.event.location;
        m.eventMessage.joinLink = m.eventMessage.joinLink ?? message.event.joinLink;
    }
    ...
    else if ('inviteFollower' in message) {
        m.newsletterFollowerInviteMessage = {
            newsletterJid: message.inviteFollower.jid,
            newsletterName: message.inviteFollower.subject,
            caption: message.inviteFollower.text,
        ...
        if (requestPayment === null || requestPayment === void 0 ? void 0 : requestPayment.sticker) {
            // Fix: do not spread options into the media message object
            const stickerPrep = await (0, exports.prepareWAMessageMedia)({ sticker: requestPayment.sticker }, options);
            notes = {
                stickerMessage: {
            ...
            const nm = requestPayment.noteMessage;
            // Accept plain objects and decoded proto Message objects alike:
            // require at least one recognised content field to be present.
            if (!nm.extendedTextMessage && !nm.stickerMessage) {
                throw new boom_1.Boom('Invalid requestPayment noteMessage: must contain extendedTextMessage or stickerMessage', { statusCode: 400 });
            }
            notes = nm;
        }
        else {
        ...
        // Coerce Long / string amounts to a plain JS number (handles forwarded messages)
        const rawAmount = requestPayment.amount ?? requestPayment.amount1000;
        const amount1000 = (rawAmount !== null && rawAmount !== undefined && typeof rawAmount === 'object' && typeof rawAmount.toNumber === 'function')
            ? rawAmount.toNumber()
        ...
        // from/requestFrom is optional — falls back to the chat recipient JID (mirroring WA Web behaviour)
        const requestFrom = requestPayment.from ?? requestPayment.requestFrom ?? options.recipientJid;
        if (amount1000 === undefined || amount1000 === null) {
            throw new boom_1.Boom('Invalid requestPayment fields: missing amount/amount1000', { statusCode: 400 });
        }
        if (currencyCodeIso4217 === undefined || currencyCodeIso4217 === null) {
            throw new boom_1.Boom('Invalid requestPayment fields: missing currency/currencyCodeIso4217', { statusCode: 400 });
        }
        ...
        m.cancelPaymentRequestMessage = { key: message.cancelPaymentRequest };
    }
    else if ('declinePaymentRequest' in message) {
        m.declinePaymentRequestMessage = { key: message.declinePaymentRequest };
    }
        ...
        const sp = message.sendPayment || message.sendPaymentMessage;
        ...
        m.sendPaymentMessage = Types_1.WAProto.Message.SendPaymentMessage.fromObject({
            noteMessage: sp.note
                ...
                : (sp.noteMessage ?? undefined),
            requestMessageKey: sp.requestKey ?? sp.requestMessageKey,
    ...
    if ('buttons' in message && !!message.buttons) {
        const hasNativeFlowButtons = message.buttons.some(button => {
            const type = button === null || button === void 0 ? void 0 : button.type;
            return !!(button === null || button === void 0 ? void 0 : button.nativeFlowInfo)
                || !!(button === null || button === void 0 ? void 0 : button.nativeFlow)
                || !!(button === null || button === void 0 ? void 0 : button.nativeFlows)
                || type === 4
                || type === WAProto_1.proto.Message.ButtonsMessage.Button.Type.NATIVE_FLOW;
        });
        if (hasNativeFlowButtons) {
            const buttonsMessage = {
                buttons: message.buttons.map(normalizeButtonsMessageButton)
            };
            if ('text' in message) {
                ...
                const headerTypeKey = type ? type.replace('Message', '').toUpperCase() : undefined;
                const headerType = headerTypeKey && WAProto_1.proto.Message.ButtonsMessage.HeaderType[headerTypeKey];
                buttonsMessage.headerType = headerType !== undefined
                    ? headerType
            ...
            applyContextInfoAndMentions(buttonsMessage, message);
            m = { buttonsMessage };
        }
        else {
    ...
    applyMediaAnnotations(m, message);
 ...
 * @param msg the event creation message containing eventResponses
 * @param meId your jid
...
function getAggregateResponsesInEventMessage({ eventResponses }, meId) {
    ...
    for (const update of eventResponses || []) {
        const responseType = update.response?.eventResponse ?? 'UNKNOWN';
...
/**
 * Checks whether a WAMessage (WebMessageInfo) contains a view-once media payload
 * (any of the three view-once variants).
 */
const isViewOnceMessage = (message) => {
    var _a;
    const msg = (_a = message === null || message === void 0 ? void 0 : message.message) !== null && _a !== void 0 ? _a : message;
    return VIEW_ONCE_WRAPPER_KEYS.some(k => !!(msg === null || msg === void 0 ? void 0 : msg[k]));
};
exports.isViewOnceMessage = isViewOnceMessage;
/**
 * Downloads the media contained inside a view-once message and returns it as a
 * Buffer together with metadata extracted from the inner message.
 *
 * Works with all three view-once wrapper variants:
 *   - `viewOnceMessage`
 *   - `viewOnceMessageV2`
 *   - `viewOnceMessageV2Extension`
 *
 * @param message   The full WebMessageInfo (or its `.message` field) that contains
 *                  a view-once wrapper.
 * @param options   Optional download options forwarded to `downloadContentFromMessage`.
 * @returns         An object with:
 *                  - `buffer`     – the raw media bytes
 *                  - `mediaType`  – e.g. `'image'`, `'video'`, `'audio'`, `'document'`, `'sticker'`
 *                  - `mimetype`   – MIME type string from the inner message proto
 *                  - `message`    – the unwrapped inner message object (contains caption, etc.)
 * @throws  Boom 400 if the message is not a view-once message or contains no media.
 */
const readViewOnce = async (message, options = {}) => {
    const msgProto = (message === null || message === void 0 ? void 0 : message.message) !== undefined ? message.message : message;
    if (!isViewOnceMessage(msgProto)) {
        throw new boom_1.Boom('Message is not a view-once message', { statusCode: 400, data: message });
    }
    const inner = (0, exports.extractMessageContent)(msgProto);
    if (!inner) {
        throw new boom_1.Boom('No inner message content found in view-once wrapper', { statusCode: 400, data: message });
    }
    const contentType = (0, exports.getContentType)(inner);
    if (!contentType) {
        throw new boom_1.Boom('Could not determine media type of view-once message', { statusCode: 400, data: message });
    }
    ...
    if (!media || typeof media !== 'object' || (!('url' in media) && !('directPath' in media))) {
        throw new boom_1.Boom('View-once message does not contain downloadable media', { statusCode: 400, data: message });
    }
    ...
    return {
        buffer,
        mediaType,
        mimetype: media.mimetype || MIMETYPE_MAP[mediaType] || 'application/octet-stream',
        message: inner,
    };
};
exports.readViewOnce = readViewOnce;
