...
const normalizeEarFields = (ear) => {
    const result = { ...ear };
    if (result.thumbnail_url && !result.thumbnailUrl) result.thumbnailUrl = result.thumbnail_url;
    if (result.thumbnailUrl && !result.thumbnail) result.thumbnail = result.thumbnailUrl;
    if (result.source_url && !result.sourceUrl) result.sourceUrl = result.source_url;
    if (result.media_type !== undefined && result.mediaType === undefined) result.mediaType = result.media_type;
    if (result.show_ad_attribution !== undefined && result.showAdAttribution === undefined) result.showAdAttribution = result.show_ad_attribution;
    if (result.render_larger_thumbnail !== undefined && result.renderLargerThumbnail === undefined) result.renderLargerThumbnail = result.render_larger_thumbnail;
...
const normalizeQuickReplyButton = (button) => {
    var _a, _b, _c;
    const displayText = (typeof button.text === 'string' && button.text)
        || (typeof button.displayText === 'string' && button.displayText)
        || (typeof button.display_text === 'string' && button.display_text)
        || (typeof button.buttonText === 'string' && button.buttonText)
        || (button.buttonText && typeof button.buttonText === 'object'
            ? (((_a = button.buttonText) === null || _a === void 0 ? void 0 : _a.displayText) || ((_b = button.buttonText) === null || _b === void 0 ? void 0 : _b.display_text))
            : '')
        || '';
    const id = button.buttonId || button.id || ((_c = button.buttonParamsJson) === null || _c === void 0 ? void 0 : _c.id) || '';
    return {
        name: 'quick_reply',
