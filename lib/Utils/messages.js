...
const DEFAULT_NEWSLETTER_POLYGON_VERTICES = [
    { x: 58.651251792907715, y: -37.128740310668945 },
    { x: -18.775586128234863, y: 48.53278160095215 },
    { x: -58.65124988555908, y: 37.128740310668945 },
    { x: 18.77558422088623, y: -48.53278160095215 },
];
...
const applyDefaultNewsletterAnnotation = (m, message) => {
    if ('defaultNewsletterAnnotation' in message && message.defaultNewsletterAnnotation === false) {
        return;
    }
    ...
    if (!m[messageType] || typeof m[messageType] !== 'object') {
        return;
    }
    const existingAnnotations = Array.isArray(m[messageType].interactiveAnnotations)
        ? m[messageType].interactiveAnnotations
        : [];
    m[messageType].interactiveAnnotations = [...existingAnnotations, DEFAULT_NEWSLETTER_ANNOTATION];
};
