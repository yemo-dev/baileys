...
const transferDevice = (fromJid, toJid) => {
    ...
    if (!toDecoded)
        return toJid;
    return (0, exports.jidEncode)(toDecoded.user, toDecoded.server, deviceId);
};
...
const lidToJid = (jid) => {
    ...
    if (jid.endsWith('@hosted.lid'))
        return jid.replace('@hosted.lid', '@hosted');
    return jid;
};
